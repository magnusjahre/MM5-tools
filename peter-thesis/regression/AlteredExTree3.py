...
def main():
    ...
    lin_write_stall = linear_model.LinearRegression()
    lin_pr_block_stall = linear_model.LinearRegression()
    lin_empty_rob_stall = linear_model.LinearRegression()
    stalls_x = stalls[regression_input]
    stalls_write = stalls['Alone Write Stall Cycles'].astype('float64')
    stalls_pr_block = stalls['Alone Private Blocked Stall Cycles'].astype('float64')
    stalls_empty_rob = stalls['Alone Empty ROB Stall Cycles'].astype('float64')
    stalls_write = pd.DataFrame(stalls_write)
    stalls_pr_block = pd.DataFrame(stalls_pr_block)
    stalls_empty_rob = pd.DataFrame(stalls_empty_rob)
    lin_write_stall = lin_write_stall.fit(stalls_x, stalls_write)
    lin_empty_rob_stall = lin_empty_rob_stall.fit(stalls_x, stalls_empty_rob)
    lin_pr_block_stall = lin_pr_block_stall.fit(stalls_x, stalls_pr_block)
    ...
    for filename in sorted(glob.glob(testFiles)):
        ...
        for index, row in data.iterrows():
            ...
            if prediction[0][0] < 0:
                prediction[0][0] = 0
            stalls_write = lin_write_stall.predict([x])
            stalls_pr_block = lin_pr_block_stall.predict([x])
            stalls_empty_rob = lin_empty_rob_stall.predict([x])
            ...
            predictions[index] = comm_insts / (prediction[0] + stalls_write[0] + stalls_pr_block[0] + stalls_empty_rob[0] + mem_ind_stalls + compute_cycles)
