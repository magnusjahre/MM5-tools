...
def main():
    ...
    for tracecontent in tracecontents:
        ...
        if (numCols < 50):
            tracecontent.headers[48] = "LLC Miss/WBs"
            tracecontent.headers[49] = "Total LLC Miss/WBs"
            tracecontent.headers[50] = "Private Tot. Lat"
            tracecontent.headers[51] = "Measured Al. Mem. Lat"
            tracecontent.headers[52] = "Measured Sh/Pr Mem Stalls"
        
    
    combinedRequests = [0] * tracecontents[0].getNumRows()
    for tracecontent in tracecontents:
        coreCombinedRequests = [0] * tracecontent.getNumRows()
        coreCombinedRequests = [m - n for m,n in zip(coreCombinedRequests, tracecontent.getColumn(45))]
        coreCombinedRequests = [m + n for m,n in zip(coreCombinedRequests, tracecontent.getColumn(46))]
        coreCombinedRequests = [m + n for m,n in zip(coreCombinedRequests, tracecontent.getColumn(47))]
        combinedRequests = [m + n for m,n in zip(coreCombinedRequests, combinedRequests)]
        tracecontent.data[48] = coreCombinedRequests
    
    for tracecontent in tracecontents:
        tracecontent.data[49] = combinedRequests
    ...
    for i in range(len(paths)):
        for filename in glob.glob(paths[i]):
            ...
            tracecontents[i].data[50] = aloneTraceContent.getColumn(12)
            tracecontents[i].data[51] = aloneTraceContent.getColumn(19)
            tracecontents[i].data[52] = aloneTraceContent.getColumn(5)
