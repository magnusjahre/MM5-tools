...
def get_command(np, benchmark, coherenceProtocol, interconnect, L1mshrCount, L2mshrCount, L1targets, L2targets):
    arguments = []
    ...
    arguments.append('-ESTATSFILE='+pbsconfig.get_unique_id(np, benchmark, L1mshrCount, L2mshrCount, L1targets, L2targets)+'.txt')
    ...
    if pbsconfig.useAdaptiveMHA:
        arguments.append('-EUSE-ADAPTIVE-MHA')
        arguments.append('-EADAPTIVE-MHA-HIGH-THRESHOLD='+str(pbsconfig.adaptiveMHAHighThreshold))
        arguments.append('-EADAPTIVE-MHA-LOW-THRESHOLD='+str(pbsconfig.adaptiveMHALowThreshold))
            ...
            for L2mshrCount in pbsconfig.l2mshrs:
                for L1targets in pbsconfig.l1mshrTargets:
                    for L2targets in pbsconfig.l2mshrTargets:
                        ...
                        fileID = pbsconfig.get_unique_id(np, benchmark, L1mshrCount, L2mshrCount, L1targets, L2targets)
                        pbsfilename = fileID+".pbs"
        
                        command = get_command(np, benchmark, 'none', 'crossbar', L1mshrCount, L2mshrCount, L1targets, L2targets)
