...
for bm in bms:
    ...
    bestVal = 0
    for k in keys:
        if results[k][bm] > bestVal:
            bestVal = results[k][bm]
            bestKey = k
    bmBest[bm] = (bestKey, bestVal)
    numWithBest[bestKey] = numWithBest[bestKey] + 1
    
for bm in bms:
    print str(bm).ljust(numwidth)+str(bmBest[bm][0]).rjust(numwidth)+str(bmBest[bm][1]).rjust(numwidth)
...
for k in keys:
    print str(k).ljust(numwidth)+str(numWithBest[k]).rjust(numwidth)
print
